...
def vespagram(stream, ev, inv, method, scale, nthroot=1,
              static3D=False, vel_corr=4.8, sl=(0.0, 10.0, 0.1),
              plot_trace=True, phase_shift=0.,
              plot_max_beam_trace=False, save_fig=False, plot_circle_path=False,
              plot_stations=False,vespa_iter=0, component ='Z'):
    """
    ...
    if i > len(seismo):
        print'Number of stations: ',len(seismo)
        print'Number of stations found: ',i
        msg = 'There are stations duplicates!!!!'
        raise ValueError(msg)
    ...
    if plot_stations:
       plot_array_stations(sz,center_lon,center_lat)
    ...
    kwargs = dict(
        ...
        sll=sll, slm=slm, sls=sls, baz=baz, stime=starttime, etime=endtime, method=method,
    ...
    fig = plt.figure(num=1,figsize=(24,10),dpi=100)
...
#    print(sls)
    ...
    if plot_trace:
        ...
        ax1.set_ylabel('slowness [s/deg]',size=24)
    ...
    #####
        ...
        ax1.scatter(phase_time_info,phase_slowness_info,s=2000,marker=u'|',lw=2,color='g')
        for i, txt in enumerate(phase_name_info):
            ax1.annotate(txt,(phase_time_info[i],phase_slowness_info[i]),fontsize=26,color='r')
...
def get_taupy_points(center_lat,center_lon,ev_lat,ev_lon,ev_depth,
  ...
  arrivals = earthmodel.get_pierce_points(ev_depth,distance,phase_list=["P","PP","P^410P","P^660P"])
...
def plot_array_stations(stream,center_lon,center_lat):
