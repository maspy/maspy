...
def vespagram(stream, ev, inv, method,isinv,isev, scale, nthroot=4, static3D=False, vel_corr=4.8, 
    ...
    static_correction=False, vespagram_show=True,station_file="None",):
    """
    ...
    :type method: str
    :param isinv: : True when you are using "STATIONXML" receiver file
                    False when you want to run vespagram using
                    a synthetic array made with instaseis.Receiver()
    :param isev: : True when you are using "XML" source file
                    False when you want to run vespagram using
                    a synthetic array made with instaseis.Source()
    False when you want to run vespagram using
    a synthetic source made with instaseis.Source()
    ...
    '''
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Changed by Stephanie Durand June 2016
    Enable to use synthetic source from instaseis
    generated by source = instaseis.Source(...)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    '''
    if not isinv:
        latitude_val=[]
        ...
        with open(station_file, "rt") as fh:
            for line in fh:
                values = re.split(" ", line) # read each line values
                station_name.append(values[0])
                latitude_val.append(float(values[1]))
                longitude_val.append(float(values[2]))
    ...
    if isev:
        ...
        ev_lat = ev.latitude
        ev_lon = ev.longitude
        ev_depth = ev.depth_in_m/1000
        ev_otime = ev.origin_time
    ...
    if isinv:
        ...
        for i_network in range(0,net): 
            for tr in seismo:
                for kk in range( len(latitude_val)):
                    if inv[k].station == station_name[kk]:
                        tr.stats.coordinates = \
                            AttribDict({'latitude': inv[k].latitude,
                                        'longitude': inv[k].longitude,
                                        'elevation': 0.})
                        center_lon += inv[k].longitude
                        center_lat += inv[k].latitude
                        center_elv += 0.
                        i += 1
                        if (i==1):
		                    max_sampling_rate = tr.stats.sampling_rate
    ...
    if sampling_rate_count > 0.: # resample the data
...
def vespagram_baz(stream, sll, slm, sls, baz, stime, etime, source_depth, distance, static_correction,
    ...
    #print len(stream), len(stream.select(sampling_rate=fs))
...
def show_distance_plot(stream, event, inventory, starttime, endtime,isinv,isev,
    ...
    if isev:
        ...
        event_depth_in_km = event.depth_in_m / 1000.0
...
def array_analysis_helper(stream, ev, inventory, method, frqlow, frqhigh,isinv,isev,
    ...
    if isev:
        ...
        ev_lat = ev.latitudeget_geometry
        ev_lon = ev.longitude
        ev_depth = ev.depth_in_m/1000.
        ev_otime = ev.origin_time
    ...
    # theoretical baz and az between source and midpoint of array
    great_circle_dist, teor_baz, az2 = gps2dist_azimuth(center_lat,center_lon,ev_lat,ev_lon)
    great_circle_dist_deg = great_circle_dist/1000/KM_PER_DEG
    print 'teor_baz',teor_baz
    print 'az', az2
    print 'gcd: ', great_circle_dist
    print 'gcd in deg: ', great_circle_dist_deg 
    #if teor_baz >= 180:
	#   teor_baz = teor_baz
    #else:
    #   teor_baz = teor_baz+360
    ...
    if isinv:
        ...
        for tr in stream:
            for k in range(0, len(inventory)):
                tr.stats.coordinates = \
                        AttribDict(dict(latitude=inventory[k].latitude,
                                        longitude=inventory[k].longitude,
                                        elevation=inventory[k].elevation))
                break
    ...
    try:
        ...
        if method == 'FK':
            ...
            print "Array processing total time in routine: %f\n" % (UTCDateTime() - start)
            # make output human readable, adjust backazimuth to values
            # between 0 and 360
            t, rel_power, abs_power, baz, slow = out.T
            baz = np.array([teor_baz]) ##########ADDED
            #print 't, rel_power, abs_power, baz, slow', t, rel_power, abs_power, baz, slow
        ...
        slmy *= KM_PER_DEG
        sls *= KM_PER_DEG
        print 'slow, sllx, slmx, slly, slmy, sls are ', slow, sllx, slmx, slly, slmy, sls 
        ...
        if baz_plot:
            ...
            bzs = np.arctan2(sls, np.sqrt(slmx*slmx + slmy*slmy))*180/np.pi # baz stepsize
            xi = np.arange(0., maxslowg, sls)
            # yi = np.arange(-180., 180., bzs)
            yi = np.arange(0, 360, bzs)
        ...
        phase_time_info = []
        slowness_info = []
        phases_list = ["Pdiff", "PKiKP", "PP", "PcP"] # Select the phases that you want to see
        ...
        for i in xrange(numslice):
            ...
            print 'baz is ', baz
            print 'slow_x and slow_y are ', slow_x, slow_y
            ...
            if method == 'FK':
                ...
                # print'T',T
                spl_plot = spl[0]
                times_stream = spl[0].times() + (spl[0].stats.starttime - ev_otime)
                print(spl)
                print 'spl[0].times are ', spl_plot.times()
                ax1.plot(times_stream, spl_plot.data, 'k')
                # ax1.plot(spl_plot.times(), spl_plot.data, 'k')
                ax1.set_xlim(np.min(times_stream), np.max(times_stream))
                ...
                print 'phase_time_info is: ', phase_time_info
                # for i, txt in enumerate(phase_name_info):
            ...
            if baz_plot:
                ...
                # np.savetxt('/Users/AnneSophieReiss/Documents/Doktorarbeit/OBSPY/DATA_NEW_INDIA/test.txt',grid, delimiter='')
                slow_back_plot = ax.pcolormesh(xi, yi, grid, cmap=cmap)
                #slow_back_plot = ax.pcolorfast(xi, yi, grid)
                #slow_back_plot = ax.pcolormesh(xi, yi, grid, cmap=cmap, vmin=0., vmax=0.1) # set colorbar limits!
                ...
                perc_see = 0.5 # percentage that you want to see
                y_low = (teor_baz)* (1-perc_see)
                y_high = (teor_baz)* (1+perc_see)
                print 'y_low and y_high are: ', y_low, y_high
                #y_low = (teor_baz-360)* (1-perc_see)
                #y_high = (teor_baz-360)* (1+perc_see)

                # plot the theoretical backazimuth and theoretical slowness
                PLOT_SLOW_BACKAZ_LINES = True
                if PLOT_SLOW_BACKAZ_LINES:
                    ax.hold(True)
                    #ax.plot((0.,14.),((teor_baz-360),(teor_baz-360)),'--',lw=4,color='k') # theoretical backazimuth line
                    ax.plot((0.,14.),((teor_baz),(teor_baz)),'--',lw=4,color='k') # theoretical backazimuth line
                    ...
                    for ii, txt in enumerate(phase_name_info): # print phase info
                        ax.annotate(txt,(slowness_info[ii],(teor_baz)),fontsize=40,color='r')
...
def get_phases_arrivals(trace, phases_list, ev_lat, ev_lon,ev_depth,
    ...
    arrivals = model.get_pierce_points(ev_depth, distance, phase_list=phases_list)
